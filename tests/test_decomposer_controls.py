...
@pytest.mark.asyncio
async def test_decompose_skips_task_seeding_when_disabled() -> None:
    d = _mk_decomposer()

    async def fake_model_decompose(task: str, max_queries: int) -> list[QuerySpec]:
        return [QuerySpec(query="base", query_type=QueryType.SEMANTIC, importance=0.7)]

    d._model_decompose = fake_model_decompose
    d._fallback_decompose = lambda task, max_queries: []
    d._apply_path_hints = lambda task, specs: specs

    calls = {"inject": 0}

    def fake_inject(task: str, specs: list[QuerySpec]) -> list[QuerySpec]:
        calls["inject"] += 1
        return specs + [QuerySpec(query="seed", query_type=QueryType.GREP, importance=1.0)]

    d._inject_task_specific_specs = fake_inject

    out = await d.decompose("mcp task", max_queries=5, use_task_seeding=False)
    assert calls["inject"] == 0
    assert all(s.query != "seed" for s in out)


@pytest.mark.asyncio
async def test_decompose_applies_task_seeding_when_enabled() -> None:
    d = _mk_decomposer()
    ...
    out = await d.decompose("mcp task", max_queries=5, use_task_seeding=True)
    assert calls["inject"] == 1
    assert any(s.query == "seed" for s in out)
